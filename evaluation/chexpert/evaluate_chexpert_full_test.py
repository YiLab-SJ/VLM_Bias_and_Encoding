"""
evaluate_chexpert_full_test.py

Evaluate CheXpert-trained probes on the FULL CheXpert test split.
Evaluates:
  1. Demographics (sex, age, ethnicity): Overall AUC + bootstrap CI,
     per-class metrics, conditioned metrics by other demographics.
  2. No Finding (binary disease): Overall AUC + bootstrap CI,
     binary metrics (TPR/FPR/TNR/FNR), conditioned by demographics.

Output directories (separate from MIMIC results):
  - evaluation_results/{model}/evaluation_results_{modality}_full_test_chexpert/
  - evaluation_results/{model}/evaluation_results_{modality}_nofinding_full_test_chexpert/

Usage:
  python evaluate_chexpert_full_test.py --n_jobs 24
  python evaluate_chexpert_full_test.py --models medversa chexzero --n_jobs 24
"""
...
def bootstrap_auroc_ci_1d(y_true, y_pred_proba_1d, n_bootstraps=1000, random_state=42):
    """Bootstrap AUC for a single binary label (1D probabilities)."""
    np.random.seed(random_state)
    scores = []
    n_samples = len(y_true)
...
def calculate_binary_metrics(y_true, y_pred_labels, y_pred_proba=None):
    """Calculate AUC, TPR, FPR, TNR, FNR for binary predictions."""
...
# =============================================================================
# DEMOGRAPHIC EVALUATION (per layer)
# =============================================================================

def _process_layer_demographics(layer_name, model, modality, feat_dir, model_dir,
                                y_test_df, demographics, log_prefix):
    """Process one layer: evaluate all demographic probes."""
    import warnings
    ...
    for attribute_probe in demographics.keys():
        ...
        # Tier 3: Conditioned metrics (single-attribute)
...
# =============================================================================
# NO FINDING EVALUATION (per layer)
# =============================================================================

def _process_layer_nofinding(layer_name, model, modality, feat_dir_test, feat_dir_val,
                             model_dir, y_test_df, y_val_df, demographics, log_prefix):
    """Process one layer: evaluate No Finding probe with conditioned metrics."""
    import warnings
    warnings.filterwarnings("ignore")
    ...
    try:
        ...
        y_val_nf = y_val_df['No Finding'].values.astype(int)
    ...
    y_true_nf = y_test_df['No Finding'].values.astype(int)
...
def evaluate_demographics(model, modality, n_jobs):
    """Evaluate demographic probes for one model/modality on CheXpert test."""
    log_prefix = f"[{model} | {modality.upper()} | DEMO]"
    logging.info(f"{log_prefix} Starting CheXpert demographic evaluation...")
    ...
    out_dir = os.path.join(RESULTS_BASE, model, f"evaluation_results_{modality}_full_test_chexpert")
    ...
    demographics = {attr: sorted(y_test_df[attr].dropna().unique()) for attr in DEMO_ATTRS}
    logging.info(f"{log_prefix} Test set: {len(y_test_df)} samples.")
    ...
    results = Parallel(n_jobs=n_jobs)(
        delayed(_process_layer_demographics)(
            layer, model, modality, feat_dir, model_dir, y_test_df, demographics, log_prefix
        ) for layer in layers
    )
...
def evaluate_nofinding(model, modality, n_jobs):
    """Evaluate No Finding probe for one model/modality on CheXpert test."""
    log_prefix = f"[{model} | {modality.upper()} | NF]"
    logging.info(f"{log_prefix} Starting CheXpert No Finding evaluation...")
    ...
    model_dir = os.path.join(exp_dir, f"trained_probes_{modality}_only")
    out_dir = os.path.join(RESULTS_BASE, model, f"evaluation_results_{modality}_nofinding_full_test_chexpert")
    ...
    demographics = {attr: sorted(y_test_df[attr].dropna().unique()) for attr in DEMO_ATTRS}
    logging.info(f"{log_prefix} Test: {len(y_test_df)}, Val: {len(y_val_df)} samples.")
    ...
    if all_results:
        timestamp = pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")
        out_file = os.path.join(out_dir, f"nofinding_full_test_{modality}_{DATASET}_{timestamp}.json")
        with open(out_file, 'w') as f:
            json.dump(convert_numpy_types(all_results), f, indent=4)
        logging.info(f"{log_prefix} Saved: {out_file}")
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate CheXpert probes (demographics + No Finding)")
    parser.add_argument('--models', nargs='+', default=TARGET_MODELS,
                        choices=TARGET_MODELS, help='Models to evaluate')
    parser.add_argument('--n_jobs', type=int, default=24,
                        help='Number of parallel layer jobs (default: 24)')
    ...
    logging.info("=" * 70)
    logging.info("  CHEXPERT FULL TEST EVALUATION")
    ...
    logging.info(f"  Output: evaluation_results/{{model}}/..._chexpert/")
    logging.info("=" * 70)

    for model in args.models:
        for modality in args.modalities:
            evaluate_demographics(model, modality, n_jobs=args.n_jobs)
            evaluate_nofinding(model, modality, n_jobs=args.n_jobs)

    logging.info("CheXpert evaluation pipeline completed.")
