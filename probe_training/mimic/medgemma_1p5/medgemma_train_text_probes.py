# medgemma_train_text_probes.py
# Trains a single text-layer demographic probe for MedGemma 1.5.
# Adapted from biovilt_train_text_probes.py with config_medgemma import.
#
# Usage:
#   python medgemma_train_text_probes.py --dataset_folder_name MIMIC-CXR-JPG \
#       --layer_name Txt_Block_34 --attribute sex
...
def train_single_text_probe():
    parser = argparse.ArgumentParser(description="Train a single MedGemma 1.5 text-layer probe.")
    ...
    parser.add_argument('--layer_name', type=str, required=True,
                        help="Text layer to probe (e.g., Txt_Block_34).")
    ...
    logging.info(f"{log_prefix} Starting MedGemma 1.5 text probe training.")
