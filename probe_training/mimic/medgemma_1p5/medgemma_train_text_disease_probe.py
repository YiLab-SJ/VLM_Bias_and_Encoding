# medgemma_train_text_disease_probe.py
# Trains multi-label disease probes on MedGemma 1.5 text (Gemma3 decoder) layer embeddings.
# Adapted from biovilt_train_text_disease_probe.py with config_medgemma import.
#
# Usage:
#   python medgemma_train_text_disease_probe.py --dataset_folder_name MIMIC-CXR-JPG \
#       --layer_name Txt_Block_34
...
from config_medgemma import (
    ...
    TEXT_LAYER_DIMS, DISEASE_LABELS
)
...
def train_text_disease_probes():
    parser = argparse.ArgumentParser(description="Train MedGemma 1.5 text disease probes for a single layer.")
    ...
    parser.add_argument('--layer_name', type=str, required=True,
                        help="Text layer to probe.")
    args = parser.parse_args()

    layer_name = args.layer_name
    dataset_folder_name = args.dataset_folder_name

    log_prefix = f"[Text Disease | {layer_name}]"
    ...
    models_dir = os.path.join(features_root_dir, "trained_probes_text_diseases")
    results_dir = os.path.join(features_root_dir, "results_text_only_disease_gridsearch")
    ...
    if os.path.exists(model_path) and os.path.exists(scaler_path) and os.path.exists(results_path):
        logging.info(f"{log_prefix} Files already exist. Skipping training to save time and compute.")
        return
