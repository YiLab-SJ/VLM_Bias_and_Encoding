# chexagent_train_text_probes.py
# Trains a single text-layer demographic probe for CheXagent (Phi-2 decoder).
# This is the CheXagent equivalent of biovilt_train_text_probes.py.
#
# Usage:
#   python chexagent_train_text_probes.py --dataset_folder_name MIMIC-CXR-JPG \
#       --layer_name Txt_PhiBlock_32 --attribute sex
...
def train_single_text_probe():
    parser = argparse.ArgumentParser(description="Train a single CheXagent text-layer probe.")
    ...
    parser.add_argument('--layer_name', type=str, required=True,
                        help="Text layer to probe (e.g., Txt_PhiBlock_32).")
    ...
    logging.info(f"{log_prefix} Starting CheXagent text probe training.")
